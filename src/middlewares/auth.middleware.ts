...
export const auth = (...allowedRoles: string[]) => {
  return (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      let decoded;
      try {
        decoded = verifyToken(token, process.env.JWT_ACCESS_SECRET as string);
      } catch {
        throw new AppError(401, "Invalid or expired token");   // ⬅️ নতুন — raw JWT error ধরে AppError-এ convert করছি
      }
